package Examenes.Post_Temas.Enero_2023.CodigoSolucion.utils;
...
public class CalendarioAnual {
    ...
    // ESTE MÉTODO LO AÑADIRÁN LOS ALUMNOS
    public boolean isReserved(String date) {
        return this.calendarioReservas.get(this.getMonth(date)).get(this.getDay(date));
    }

    // ESTE MÉTODO LO AÑADIRÁN LOS ALUMNOS
    public void reserve(String date) {
        this.calendarioReservas.get(this.getMonth(date)).put(this.getDay(date), Boolean.TRUE);
    }

    public void deleteReservation(String date) {
        this.calendarioReservas.get(this.getMonth(date)).put(this.getDay(date), Boolean.FALSE);
    }
}
